...
addEventListenerTo(document,'mousemove',function(e){
	var rx = e.clientX - Mouse.x;
	var ry = e.clientY - Mouse.y;
    Mouse.x=Mouse[0]=e.clientX;
    Mouse.y=Mouse[1]=e.clientY;
    Mouse.sx=e.screenX;
    Mouse.sy=e.screenY;
	if ('mmrel' in window) window.mmrel(rx, ry);
});
...
addEventListenerTo(document,'mousedown',function(e){
    Mouse[['down','mmbdown_','rmbdown'][e.button]]=true;
});

addEventListenerTo(document,'mouseup',function(e){
    Mouse[['down','mmbdown_','rmbdown'][e.button]]=false;
});

window.Mouse = {x:0, y:0, 0:0, 1:0, sx:0, sy:0, down:false, mmbdown_: false, rmbdown:false, mmbemu: false, get mmbdown() {
	return this.mmbdown_ || (this.rmbdown && this.down && this.mmbemu);
}};
